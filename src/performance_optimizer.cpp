...
void PerformanceOptimizer::init(bool use_huge_pages, int io_threads, int cpu_threads) {
    ...
    if (use_huge_pages && is_huge_pages_available()) {
        ...
        Logger::info(_fmt("Huge Pages enabled ({} MB pages)", "Huge Pages включены (страницы {} МБ)", huge_page_size_ / (1024 * 1024)));
    ...
    Logger::info(_fmt("Performance optimizer initialized: {} I/O threads, {} CPU threads", "Оптимизатор производительности инициализирован: {} I/O потоков, {} CPU потоков", io_thread_count_, cpu_thread_count_));
}
...
bool PerformanceOptimizer::set_cpu_affinity(int core_id) {
    ...
    if (core_id < 0 || core_id >= get_cpu_count()) {
        Logger::warning(_fmt("Invalid CPU core ID: {}", "Неверный ID ядра CPU: {}", core_id));
        return false;
    }
    ...
    if (sched_setaffinity(0, sizeof(cpuset), &cpuset) != 0) {
        Logger::error(_fmt("Failed to set CPU affinity to core {}: {}", "Не удалось установить привязку CPU к ядру {}: {}", core_id, strerror(errno)));
        return false;
    }
    
    Logger::debug(_fmt("CPU affinity set to core {}", "Привязка CPU установлена к ядру {}", core_id));
    return true;
}
...
bool PerformanceOptimizer::set_thread_cpu_affinity(std::thread& thread, int core_id) {
    ...
    if (core_id < 0 || core_id >= get_cpu_count()) {
        Logger::warning(_fmt("Invalid CPU core ID for thread: {}", "Неверный ID ядра CPU для потока: {}", core_id));
        return false;
    }
    ...
    if (pthread_setaffinity_np(native_handle, sizeof(cpuset), &cpuset) != 0) {
        Logger::error(_fmt("Failed to set thread CPU affinity to core {}: {}", "Не удалось установить привязку потока CPU к ядру {}: {}", core_id, strerror(errno)));
        return false;
    }
    
    Logger::debug(_fmt("Thread CPU affinity set to core {}", "Привязка потока CPU установлена к ядру {}", core_id));
    return true;
}
...
PerformanceOptimizer::AllocatedMemory PerformanceOptimizer::allocate_aligned_memory(size_t size, bool use_huge_page) {
    ...
    if (use_huge_page && huge_pages_initialized_) {
        ...
        if (ptr == MAP_FAILED) {
            Logger::warning(_fmt("Huge Pages allocation failed: {}, falling back to regular pages", "Выделение Huge Pages не удалось: {}, переход на обычные страницы", strerror(errno)));
            ptr = nullptr;
        } else {
            Logger::debug(_fmt("Allocated {} bytes with Huge Pages", "Выделено {} байт с Huge Pages", aligned_size));
            method = 1;
    ...
    if (ptr == nullptr) {
        if (posix_memalign(&ptr, page_size_, aligned_size) != 0) {
            Logger::error(_fmt("Failed to allocate aligned memory: {}", "Не удалось выделить выровненную память: {}", strerror(errno)));
            return {nullptr, 0, 0};
        }
        Logger::debug(_fmt("Allocated {} bytes with regular pages", "Выделено {} байт обычными страницами", aligned_size));
        method = 0;
    }
...
bool PerformanceOptimizer::preallocate_file(int fd, uint64_t size) {
    ...
    if (fallocate(fd, 0, 0, size) != 0) {
        ...
        if (ftruncate(fd, size) != 0) {
            Logger::warning(_fmt("Failed to preallocate file: {}", "Не удалось предварительно выделить файл: {}", strerror(errno)));
            return false;
        }
    }
    
    Logger::debug(_fmt("Preallocated {} bytes on FD {}", "Предварительно выделено {} байт на FD {}", size, fd));
    return true;
}
...
bool PerformanceOptimizer::advise_file_access(int fd, bool sequential, bool no_reuse, bool write_once) {
    ...
    if (posix_fadvise(fd, 0, 0, advice) != 0) {
        ...
        Logger::debug(_fmt("posix_fadvise failed (non-critical): {}", "posix_fadvise завершилась неудачно (не критично): {}", strerror(errno)));
        return false;
    }
    
    Logger::debug(_fmt("File access advice set: sequential={}, no_reuse={}, write_once={}", "Установлена рекомендация доступа: последовательно={}, не переиспользовать={}, однократная запись={}", sequential, no_reuse, write_once));
    return true;
}
